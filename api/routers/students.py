...
import re
from api import main, models, schemas

router = APIRouter()
PASSWORD_REGEX = r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$"

@router.post("/signup", status_code=status.HTTP_201_CREATED, response_model=schemas.userCheck)
async def newUser(user:schemas.newUser, db: Session = Depends(main.get_db)) -> schemas.userCheck:
...
async def getUser(id: int, db: Session = Depends(main.get_db)) -> schemas.userCheck:
